...
color_list = ['green', 'blue', 'red', 'cyan', 'magenta', 'yellow', 'black', 'purple', 'orange', 'grey', 'brown']

# Assuming 'grouped_df' has country names as column headers
countries = grouped_df.columns.tolist()

# Create a dictionary to map each country to a specific color
color_map = {country: color_list[i % len(color_list)] for i, country in enumerate(countries)}
...
def plot_cocoa_production(grouped_df, country, yoy_data):
    ...
    """
    color = color_map[country]  # Get the color from the color map
